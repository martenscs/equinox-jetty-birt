...
public class SampledbPlugin extends BIRTPlugin {
	private static final Logger logger = Logger.getLogger(SampledbPlugin.class
			.getName());
	...
	public void stop(BundleContext context) throws Exception {
		...
		logger.info("Sampledb plugin stopping. Current startCount="
				+ startCount);
		synchronized (SampledbPlugin.class) {
			if (startCount >= 1) {
				if (--startCount == 0) {
	...
	private void init() throws IOException {
		...
		URL fileURL = PluginResourceLocator.getPluginEntry(
				SampleDBConstants.PLUGIN_ID, dbEntryName);

		if (fileURL == null) {
			fileURL = this.getClass().getClassLoader()
					.getResource(SAMPLE_DB_JAR_FILE);
			if (fileURL == null) {
	...
	private void shutdownDerby() {
		...
		try {
			cf = JDBCDriverManager.getInstance().getDriverConnectionFactory(
