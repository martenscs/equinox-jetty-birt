...
public class SampleDBJDBCConnectionFactory implements IConnectionFactory {
	private static final Logger logger = Logger
			.getLogger(SampleDBJDBCConnectionFactory.class.getName());
	...
	public Connection getConnection(String driverClass, String url,
		...
		if (!url.equals(SampleDBConstants.DRIVER_URL)) {
			...
			throw new SQLException(
					"Classic Models Inc. Sample Database Driver does not recognize url: "
							+ driverClass);
		}
	...
	/**
	 * Sets up the thread context class loader to make sure that Derby works
	...
	private synchronized void initClassLoaders() {
		...
		/*
		 * if ( derbyClassLoader.isGood() ) { ClassLoader contextLoader =
		 * Thread.currentThread().getContextClassLoader(); if ( contextLoader !=
		 * null && ! ( contextLoader instanceof ContextClassLoaderDelegator) ) {
		 * // Replace context loader with a wrapper
		 * Thread.currentThread().setContextClassLoader( new
		 * ContextClassLoaderDelegator( contextLoader, derbyClassLoader ) ); } }
		 */
	}
		...
		if (derbyDriver == null) {
			try {
				derbyDriver = (Driver) Class.forName(
						SampleDBConstants.DERBY_DRIVER_CLASS, true,
						this.getClass().getClassLoader()).newInstance();
			} catch (Exception e) {
				logger.log(Level.WARNING,
						"Failed to load Derby embedded driver: "
	...
	/**
	 * Class loader to delegate Derby class and resource loading to our loader,
	 * and others to the default context loader
	 */
	/*
	 ...
	 * throws ClassNotFoundException { if ( DerbyClassLoader.isDerbyClass(name)
	 ...
	 * DerbyClassLoader.isDerbyResource(name) ) { // Always delegate derby
	 * resources to derby resource loader return derbyLoader.getResource(name);
	 * } else { // Delegate to default implementation, which will use parent
	 * classloader return super.getResource( name); } } }
	 */

	/**
	 * Handles loading of Derby classes and resources to ensure that the right
	 * version of Derby library is used. Works around DERBY-1228
	 * (http://issues.apache.org/jira/browse/DERBY-1228)
	 */
	private static class DerbyClassLoader extends URLClassLoader {
		...
		private static final String NO_PLUGIN_ENTRY_LOG_MSG = "Unable to find derby.jar in " + DERBY_PLUGIN_ID + " plugin."; //$NON-NLS-1$ //$NON-NLS-2$
		...
		public static boolean isDerbyResource(String name) {
			return name.startsWith("org/apache/derby")
		...
		protected synchronized Class loadClass(String name, boolean resolve)
			...
			if (isGood && isDerbyClass(name)) {
				// All derby classes must be resolved against derby.jar in the
				// plugin
				// We will not check the parent class loader first. (this is the
				// only difference
		...
		/**
		 * @see java.lang.ClassLoader#getResource(java.lang.String) Override
		 *      this method to make sure Derby gets its resources from our Jar
		 *      (such as version info)
		 */
